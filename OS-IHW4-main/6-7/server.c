...
struct observer_info monitor_clients[MAX_CLIENTS];
sem_t observer_sem;
...
void notify_observers(const char *message) {
    ...
    for (int i = 0; i < MAX_CLIENTS; ++i) {
        if (monitor_clients[i].addr_len != 0) {
            sendto(server_fd, message, strlen(message), 0,
                   (struct sockaddr *)&monitor_clients[i].addr, monitor_clients[i].addr_len);
        }
    }
        ...
        if (n > 0) {
            ...
            if (strcmp(buffer, "OBSERVER") == 0) {
                ...
                for (i = 0; i < MAX_CLIENTS; ++i) {
                    if (monitor_clients[i].addr_len == 0) {
                        monitor_clients[i].addr = client_addr;
                        monitor_clients[i].addr_len = client_len;
                        break;
                    }
                }
                sem_post(&observer_sem);
